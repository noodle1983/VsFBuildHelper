...
$fileshare="FASTBUILD_SHARED_PATH"
$version="v1.05"
...
if (-not (Test-Path "$rootPath\fastbuild-$version.zip")) {
    copy "FASTBuild-Windows-x64-$version.zip" "$rootPath\fastbuild-$version.zip"
...
[array]$configs=gwmi win32_networkadapterconfiguration | where {$_.ipaddress -ne $null -and $_.defaultipgateway -ne $null}
if ($configs -ne $null) {
	[System.Environment]::SetEnvironmentVariable("FASTBUILD_PREFER_IP", $configs[0].IPAddress[0], [System.EnvironmentVariableTarget]::Machine)
}
